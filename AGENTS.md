...
This file provides guidance to Codex AI assistants when working with code in this repository.
...
- Request error toasts: API request failures must be surfaced once from the business catch path using `getRequestErrorMessage` / `showRequestError` from `@/utils/requestError`. Prefer backend `response.data.message`; use the frontend fallback when an Axios error has no backend message; preserve non-Axios business `Error.message` values. Do not add non-401 toast notifications in Axios interceptors, because that causes duplicate "API + system" error messages. 401 remains globally handled by the Axios interceptor, which must use a friendly session-expired fallback instead of Axios `error.message`; business catch blocks must use `showRequestError` or `isAuthRequestError` to avoid a second toast. Local validation errors that are not API failures can still call `messageClient` directly.

**Figma asset workflow**:
- When implementing Figma-driven UI, use the local Figma MCP tools to inspect the exact node before coding. Prefer narrow node requests (`get_figma_data` with the specific frame/icon node and a small depth) over loading an entire page.
- For bitmap or vector assets shown in Figma, download the original node with `download_figma_images` into a semantic frontend asset path such as `frontend/src/assets/icons/<feature>/` or `frontend/src/assets/images/<feature>/`. Do not recreate Figma icons with CSS or hand-written SVG unless the export is unusable and the fallback is intentionally documented.
- Download the smallest meaningful node. For example, export the icon/background node itself, not a parent row that also contains text. If an item list uses different icons per row, export each distinct icon node and map them explicitly in code instead of reusing a single generic icon.
- After downloading SVG assets, inspect the file before importing it. Reject or replace exports that contain invalid path data such as `nan`, unexpected text content, oversized parent-frame dimensions, or missing `width`/`height`. Broken exports must not be left untracked in the asset directory.
- Keep asset imports explicit in the component that uses them, with `alt=""` and `aria-hidden="true"` for decorative icons. Use CSS Modules only for sizing/alignment (`width`, `height`, `object-fit`, `flex`), not for redrawing the icon shape.
- If a Figma export is invalid, first retry by exporting a deeper child node. If it is still invalid, create a clean minimal SVG that matches the Figma geometry/colors, and note why the manual fallback was necessary.
